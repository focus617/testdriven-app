...
class TestUserService(BaseTestCase):
    ...
    def test_add_user(self):
        ...
        with self.client:
            ...
            self.assertIn('zhxu@163.com was added!', data['message'])
            self.assertIn('success', data['status'])
    ...
    def test_add_user_invalid_json_keys(self):
        """Ensure error is thrown if the JSON object w/o a username key."""
        with self.client:
            response = self.client.post(
                '/users',
                data=json.dumps({'email': 'zhxu@163.com'}),
                content_type='application/json',
            )
            data = json.loads(response.data.decode())
            self.assertEqual(response.status_code, 400)
    ...
    def test_single_user(self):
        ...
        user = add_user('zhxu', 'zhxu@163.com')

        with self.client:
            response = self.client.get(f'/users/{user.id}')
            data = json.loads(response.data.decode())
            self.assertEqual(response.status_code, 200)
            self.assertIn('zhxu', data['data']['username'])
            self.assertIn('zhxu@163.com', data['data']['email'])
            self.assertIn('success', data['status'])
